...
def get_query(keyword, x):
    ...
    #url = "https://openapi.naver.com/v1/search/blog?query=" + encText + "&display=100&start=" + x
...
def run_naver_search_from_blog(keyword):
    fout=open("naver_result_%s.csv" %keyword,"w",encoding="utf-8",newline="")       #파일을 쓰기모드로 열었다.
    ...
    for i in range(1,1000,100):
        ...
        for x in b["items"]:
            ...
            row_for_write.append(x["bloggername"])
            row_for_write.append(x["bloggerlink"])
            row_for_write.append(x["postdate"])
